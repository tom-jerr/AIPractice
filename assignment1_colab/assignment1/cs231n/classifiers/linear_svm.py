    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                loss += margin
                #################
                # Change the gradient calculation
                #################
                # s_ij - s_y[i] + 1求导
                # 实际上s_ij只有第j类的分类器有效；s_y[i]只有第y[i]类的分类器有效
                dW[:, j] += X[i]
                dW[:, y[i]] -= X[i]
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    margin[np.arange(num_train), y] = 0 # 将正确分类的margin置为0
    loss = np.sum(margin) / num_train + reg * np.sum(W * W) # 计算loss
