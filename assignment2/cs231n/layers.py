...
def svm_loss(x, y):
    ...
    x_true = x[np.arange(N), y][:, None]      # x[range(N), y]是一个一维向量，需要转换为二维向量
    margins = np.maximum(0, x - x_true + 1)   # margin for each score
    loss = margins.sum() / N - 1              # 将正确的类别造成的损失减去
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    # batch noramlization在N维度上进行归一化；spatial batch normalization在N,H,W三个维度上进行归一化
