...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i,(x, y) in enumerate(list(zip([input_dim] + hidden_dims, hidden_dims + [num_classes]))):
            self.params[f'W{i+1}'] = np.random.normal(0, weight_scale, (x, y))
    ...
    def loss(self, X, y=None):
        ...
        loss, x = softmax_loss(scores, y)
        for i in reversed(range(1, self.num_layers + 1)):
            loss += 0.5 * self.reg * np.sum(self.params[f'W{i}']**2)
            if i < self.num_layers:
                if self.use_dropout:
                    grad = dropout_backward(grad, cache[f'drop{i}'])
                grad = relu_backward(grad, cache[f'rL{i}'])
                if self.normalization == "batchnorm":
                    grad, grads[f'gamma{i}'], grads[f'beta{i}'] = batchnorm_backward(grad, cache[f'norm{i}'])
                if self.normalization == "layernorm":
                    grad, grads[f'gamma{i}'], grads[f'beta{i}'] = layernorm_backward(grad, cache[f'layer{i}'])

            grad, grads[f'W{i}'], grads[f'b{i}'] = affine_backward(x if i == self.num_layers else grad, cache[f'L{i}'])
